...
string_buffer::string_buffer(size_type initial_alloc_size)
...
void
string_buffer::assert_valid() const
{
  assert(bufalloc != 0);
...
void
string_buffer::reset(size_type initial_alloc_size)
{
...
void
string_buffer::remove_front(size_type len)
{
  assert(len <= size());
...
void
string_buffer::truncate(size_type len)
{
  if (len >= size()) {
    return;
  }
...
string_buffer::make_space(size_type len)
{
...
void
string_buffer::append_done(size_type len_written)
{
  buf_end += len_written;
  assert_valid();
}
...
void
string_buffer::append(const value_type *str, size_type len)
{
